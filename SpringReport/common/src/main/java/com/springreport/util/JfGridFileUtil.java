...
public class JfGridFileUtil {
    ...
    public static JSONArray getSheetByIndex(JSONObject dbObject,Long id){
        JSONArray _resultModel=null;
        if(dbObject!=null){
            if(dbObject.containsKey("celldata")){
                _resultModel=dbObject.getJSONArray("celldata");
                _resultModel.getJSONObject(0).put("id", id);
            }
        }
        return _resultModel;
    }
    ...
    public static Integer getSheetPositionByIndex(List<JSONObject> _list,String index){
        ...
        if(_list!=null ){
            for(int x=0;x<_list.size();x++){
                JSONObject _o=_list.get(x);
                if(_o.containsKey("sheet_index")){
                    try{
                        if(index.equals(_o.get("sheet_index").toString())){
                            _resultModel=x;
                            break;
                        }
                    }catch (Exception ex){
     ...
     * @return
     */
    public static Object getObjectByObject(JSONObject dbObject,String k){
        if(dbObject!=null){
            if(dbObject.containsKey(k)){
                return dbObject.get(k);
            }
        }
        return null;
    }
