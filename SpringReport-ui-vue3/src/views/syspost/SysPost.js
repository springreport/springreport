export default {
  ...
  data() {
    return{
      pageData:{
        //查询表单内容 start
        searchForm:[
					{type:'Input',label:'岗位编码',prop:'postCode'},
					{type:'Input',label:'岗位名称',prop:'postName'},
        ],
        //查询表单内容 end
        //查询条件 start
        queryData:{
        ...
        //查询表单按钮start
        searchHandle:[
          {label:'查询',type:'primary',handle:()=>this.searchtablelist(),auth:'sysPost_search'},
          {label:'重置',type:'warning',handle:()=>this.resetSearch(),auth:'sysPost_search'}
        ],
        //查询表单按钮end
        //表格数据start
        tableData:[],
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'sysPost_insert'},
          {label:'批量删除',type:'danger',handle:()=>this.deleteBatch(),auth:'sysPost_delete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        tableCols:[
					...
					{label:'操作',prop:'operation',align:'center',type:'button',btnList:[
						{label:'查看',type:'primary',auth:'sysPost_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'primary',auth:'sysPost_edit',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
						{label:'删除',type:'primary',auth:'sysPost_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
        ...
        //modal表单 start
        modalForm:[
					{type:'Input',label:'岗位编码',prop:'postCode',rules:{required:true,maxLength:50}},
					{type:'Input',label:'岗位名称',prop:'postName',rules:{required:true,maxLength:50}},
					{type:'Input',label:'排序',prop:'postSort',rules:{required:true,type:'integer'}},
        ],
        //modal表单 end
        //modal 数据 start
        modalData : {//modal页面数据
  ...
  methods:{
    ...
    getDetail(id){
      var obj = {
        url:this.apis.sysPost.getDetailApi,
        params:{id:id},
      }
      this.commonUtil.doGet(obj).then(response=>{
        ...
        this.pageData.modalData.postSort = this.pageData.modalData.postSort+"";
      });
    },
    /**
     * @description: 关闭modal
    ...
    save(){
      this.$refs['modalRef'].$refs['modalFormRef'].validate((valid) => {
        if (valid) {
            ...
            {
              obj.url = this.apis.sysPost.insertApi;
            }else{
              obj.url = this.apis.sysPost.updateApi
            }
    ...
    deleteOne(id){
      let obj = {
        url:this.apis.sysPost.deleteOneApi,
    ...
    deleteBatch(){
        ...
        if(length == 0)
        {
          ...
          let obj = {
            url:this.apis.sysPost.deleteBatchApi,
            messageContent:this.commonUtil.getMessageFromList("confirm.delete",null),
            callback:this.searchtablelist,
            params:ids,
            type:"post",
