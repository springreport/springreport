export default {
      ...
      pageData:{
        ...
        tableCols:[
					...
					{label:'操作',prop:'operation',align:'center',type:'button',width:300,fixed:'right',btnList:[
						{label:'查看',type:'primary',auth:'sysRole_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
            {label:'编辑',type:'primary',auth:'sysRole_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
            {label:'功能权限',type:'primary',auth:'sysRole_authed',handle:(row)=>this.showAuthModal(row.id)},
						{label:'报表权限',type:'primary',auth:'sysRole_reportAuth',handle:(row)=>this.showReportAuthModal(row.id)},
            {label:'删除',type:'primary',auth:'sysRole_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
        ...
        //授权页面配置end
        //报表授权页面配置 start
        reportAuthDialogParam:{//modal页面标题、是否显示等参数
          title: "权限配置", //弹窗标题,值为:新增，查看，编辑
          show: false, //弹框显示,
          type:"1",
        },
        reportAuthModalForm:[
          {type:'Tree',data:[],checked:[],props:{children: 'children',label: 'reportName'},key:"reportId",ref:"reportAuthTree"},
        ],
  ...
  methods:{
    ...
    authReport(){//确定授权
      var checkedNodes = this.$refs.reportAuthRef.$refs.reportAuthTree[0].getCheckedNodes(false,true);
      if(checkedNodes && checkedNodes.length>0)
      {
        for (let index = 0; index < checkedNodes.length; index++) {
