export default {
  ...
  data() {
    return{
      pageData:{
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'onlineTpl_insert'},
        ...
        //表格列表头start
        tableCols:[
					{label:'文档名称',prop:'tplName',align:'center'},
					{label:'操作',prop:'operation',align:'center',type:'button',btnList:[
						{label:'查看',type:'primary',auth:'onlineTpl_getDetai',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'primary',auth:'onlineTpl_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
            {label:'编辑文档',type:'primary',auth:'onlineTpl_editDoc',handle:(row)=>this.routerTo(row)},
						{label:'删除',type:'primary',auth:'onlineTpl_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
  ...
  methods:{
    ...
    getDetail(id){
      var obj = {
        url:this.apis.onlineTpl.getDetailApi,
        params:{id:id},
      }
