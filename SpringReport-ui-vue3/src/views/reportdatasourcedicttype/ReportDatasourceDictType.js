  ...
  data() {
    return{
      pageData:{
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'reportDatasourceDictType_insert'},
          {label:'批量删除',type:'danger',handle:()=>this.deleteBatch(),auth:'reportDatasourceDictType_delete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        tableCols:[
          {label:'字典类型',prop:'dictType',align:'center',overflow:true},
					...
					{label:'操作',prop:'operation',align:'center',type:'button',fixed:'right',width:220,btnList:[
						{label:'查看',type:'primary',auth:'reportDatasourceDictType_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'primary',auth:'reportDatasourceDictType_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
						{label:'删除',type:'primary',auth:'reportDatasourceDictType_delete',handle:(row)=>this.deleteOne(row.id)},
            {label:'字典值',type:'primary',auth:'reportDatasourceDictType_dictData',handle:(row)=>this.routerTo(row)},
					]}
        ],
        //表格列表头end
        //modal配置 start
  ...
  methods:{
    ...
    searchtablelist(){
      var obj = {
        ...
        params:Object.assign({datasourceId:this.$route.query.reportDatasourceId}, this.pageData.queryData, this.pageData.tablePage),
      }
    ...
    save(){
      this.$refs['modalRef'].$refs['modalFormRef'].validate((valid) => {
        if (valid) {
            var obj = {
              params:Object.assign({datasourceId:this.$route.query.reportDatasourceId}, this.pageData.modalData),
              removeEmpty:false,
            }
    ...
    routerTo(row){
      ...
      this.$router.push({ name: 'reportDatasourceDictData',query:{reportDatasourceId:row.datasourceId,dictType:row.dictType}})
    },
  }
};