export default {
    ...
    return{
      pageData:{
        ...
        searchForm:[
					...
					{type:'Select',label:'数据源类型',prop:'type',options:this.selectUtil.datasourceType},
        ],
        //查询表单内容 end
        //查询条件 start
        queryData:{
        ...
        //查询表单按钮start
        searchHandle:[
          {label:'查询',type:'primary',handle:()=>this.searchtablelist(),auth:'reportDatasource_search'},
          {label:'重置',type:'warning',handle:()=>this.resetSearch(),auth:'reportDatasource_search'}
        ],
        //查询表单按钮end
        //表格数据start
        tableData:[],
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'reportDatasource_insert'},
          {label:'批量删除',type:'danger',handle:()=>this.deleteBatch(),auth:'reportDatasource_batchDelete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        tableCols:[
					...
					{label:'操作',prop:'operation',align:'center',type:'button',width:300,fixed:'right',btnList:[
						{label:'查看',type:'primary',auth:'reportDatasource_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'primary',auth:'reportDatasource_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
            {label:'删除',type:'primary',auth:'reportDatasource_delete',handle:(row)=>this.deleteOne(row.id)},
            {label:'数据字典',type:'primary',auth:'reportDatasource_dict',handle:(row)=>this.routerTo(row)},
					]}
        ],
        //表格列表头end
        //modal配置 start
        ...
        modalForm:[
					...
					{type:'Input',label:'数据源名称',prop:'name',rules:{required:true,maxLength:40}},
					{type:'Select',label:'数据源类型',prop:'type',rules:{required:true},options:this.selectUtil.datasourceType,change:this.changeDatasourceType},
					{type:'Input',label:'数据源链接',prop:'jdbcUrl',rules:{required:true,maxLength:500}},
					{type:'Input',label:'登录名',prop:'userName',rules:{required:true,maxLength:40}},
          {type:'Password',label:'密码',prop:'password',rules:{required:true,maxLength:50}},
          {type:'Select',label:'返回值类型',prop:'apiResultType',rules:{required:true},options:this.selectUtil.apiResultType,change:this.changeResultType},
          {type:'Input',label:'返回值属性前缀',prop:'apiColumnsPrefix',rules:{required:false,maxLength:200}},
        ...
        apiResultProps:[
          ...
          {label:'操作',prop:'operation',align:'center',type:'button',width:120,btnList:[
            {label:'编辑',type:'primary',auth:'ignore',handle:(row,index)=>this.editProps(row,index)},
            {label:'删除',type:'primary',auth:'ignore',handle:(row,index)=>this.deleteProps(row,index)},
					]}
        ],
        propsTableHandles:[
        ...
        apiHeaderProps:[
          ...
          {label:'操作',prop:'operation',align:'center',type:'button',width:120,btnList:[
            {label:'编辑',type:'primary',auth:'ignore',handle:(row,index)=>this.editHeaders(row,index)},
            {label:'删除',type:'primary',auth:'ignore',handle:(row,index)=>this.deleteHeaders(row,index)},
					]}
        ],
        headersTableHandles:[
        ...
        resultPropModalForm:[
					...
					{type:'Input',label:'属性值编码',prop:'propCode',rules:{required:true,type:'letter'}},
        ],
        resultPropModalData : {//modal页面数据
					propCode:"",//编码 
					propName:"",//数据源名称 
        },
  ...
  methods:{
      ...
      this.$refs['modalRef'].$refs['modalFormRef'].validate((valid) => {
        if (valid) {
            ...
            if(this.pageData.modalData.type == "4")
            {
              ...
              if(this.pageData.modalData.apiResultType == "ObjectArray" || this.pageData.modalData.apiResultType == "Object")
              {
                if(!this.pageData.propsTableData || this.pageData.propsTableData.length == 0)
                {
                  this.commonUtil.showMessage({message:this.commonUtil.getMessageFromList("error.empty.apiresultprops",null),type: this.commonConstants.messageType.error});
                  return;
                }
    ...
    routerTo(row){
      this.$router.push({ name: 'reportDatasourceDictType',query:{reportDatasourceId:row.id}})
    },
    //修改数据源类型
