      ...
      pageData:{
        ...
        tableCols:[
          ...
          {label:'排序',prop:'sort',align:'center',overflow:true},
					{label:'操作',prop:'operation',align:'center',type:'button',width:240,fixed:'right',btnList:[
            {label:'查看',type:'primary',auth:'sysMenu_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
            {label:'编辑',type:'primary',auth:'sysMenu_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
            {label:'菜单功能',type:'primary',auth:'sysMenu_sysApi',handle:(row)=>this.routerTo(row)},
						{label:'删除',type:'primary',auth:'sysMenu_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
