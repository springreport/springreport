    ...
    return {
      ...
      pageData: {
        ...
        tableHandles: [
          ...
          { label: '导出模板', type: 'warning', position: 'right', iconClass: 'action-icon-download', handle: () => this.exportTemplate(), auth: 'wordtemplate_export' },
          { label: '导入模板', type: 'danger', position: 'right', iconClass: 'action-icon-upload', handle: () => this.importTemplate(), auth: 'wordtemplate_import' },
          {
            label: '刷新',
            type: 'danger',
            position: 'left',
  ...
  methods: {
    ...
    exportTemplate(){
      ...
      if (length == 0) {
        ...
        const obj = {
          url: this.apis.templateExportImport.templateWordImportApi,
    ...
    uploadPic(evt) {
      ...
      var that = this
      Axios.post(this.apis.templateExportImport.templateWordUploadApi , formData, config)
        .then(res => {
          if(res.status == 200)
          {//请求成功
