  ...
  data() {
    return{
      pageData:{
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新建目录',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert,null,'1'),auth:'docTpl_folder'},
          {label:'新建文档',type:'warning',handle:()=>this.showModal(this.commonConstants.modalType.insert,null,'2'),auth:'docTpl_insert'},
          {label:'刷新',type:'danger',handle:()=>this.searchtablelist(),auth:'docTpl_search'},
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        tableCols:[
          ...
          {label:'数据源名称',prop:'dataSourceName',align:'center',overflow:true},
					{label:'操作',prop:'operation',align:'center',type:'button',btnList:[
						{label:'查看',type:'primary',auth:'docTpl_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id),show:(row)=>this.isShowBtn(row)},
						{label:'编辑',type:'primary',auth:'docTpl_edit',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id,row.type)},
            {label:'设计',type:'primary',auth:'docTpl_design',handle:(row)=>this.routerTo('docDesign',row),show:(row)=>this.isShowBtn(row)},
            {label:'报表查看',type:'primary',auth:'docTpl_view',handle:(row)=>this.routerTo('docPreview',row),show:(row)=>this.isShowBtn(row)},
						{label:'删除',type:'primary',auth:'docTpl_delete',handle:(row)=>this.deleteOne(row.id,row.type)},
					]}
        ],
        //表格列表头end
        //modal配置 start
