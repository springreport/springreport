    ...
    methods:{
        ...
        getReportParam(){
            ...
            this.searchHandle=[
                ...
                {label:'打印(全部数据)',icon:'icon-printer',type:'danger',handle:()=>this.pdfPrint(1),size:'small'},
            ],
            ...
            this.commonUtil.doPost(obj,headers) .then(async response=>{
                if (response.code == "200")
                {
                        ...
                        for(let m = 0;m<result[i].params.length;m++){
                            ...
                            if(result[i].params[m].paramType == "mutiselect" || result[i].params[m].paramType == "multiTreeSelect" || result[i].params[m].componentType == "mutiselect" || result[i].params[m].componentType == "multiTreeSelect")
                            {
                                ...
                                if(this.isDrillBack == 1)
                                {
                                    if(this.parentParams && this.parentParams[dataSet.datasetId] && this.parentParams[dataSet.datasetId][result[i].params[m].paramCode])
                                    {
                                        param[result[i].params[m].paramCode] =  this.parentParams[dataSet.datasetId][result[i].params[m].paramCode];
                                    ...
                                    if(result[i].params[m].paramType == "select")
                                    {
                                        ...
                                        if(isRelyOnParams == 1 && (this.$route.query[relyOnParams] || tempParams[relyOnParams])){
                                            await that.getRelyOnParamys(result[i].params[m],this.$route.query[relyOnParams]?this.$route.query[relyOnParams]:tempParams[relyOnParams]);
                                        }
                                    }
                                }else if(this.isDrill == 1 && this.isDrillBack == 2 && this.drillParams)
                                {
                    ...
                    if(isPagination)
                    {
                        ...
                        this.searchHandle.push({btnType:'dropDown',label:'分页pdf预览',icon:'icon-file-pdf-one',type:'warning',handle:()=>this.pdfExport(),size:'small',
                        downs:[{label:'PDF预览(当前页)',handle:()=>this.pdfPageExport(1)},
                        {label:'PDF预览(指定页)',handle:()=>this.showCustomPdfPage(1)}]},)
                        this.searchHandle.push({btnType:'dropDown',label:'分页打印',icon:'icon-printer',type:'danger',handle:()=>this.pdfExport(),size:'small',
                    ...
                    this.commonUtil.doPost(obj,headers) .then(response=>{
                        if (response.code == "200")
                        {
                            ...
                            {//显示工具栏
                                ...
                                {
                                    this.searchHandle.push({label:'上报数据',icon:'icon-upload',type:'success',handle:()=>this.submitDatas(),size:'small'},);
                                    this.searchHandle.push({label:'修改记录',icon:'icon-history',type:'danger',handle:()=>this.getSubmitHis(),size:'small'},);
                                }
                            }
                            ...
                            if(response.responseData.coeditFlag == 1)
                            {
                                ...
                                this.sheetOptions.updateUrl = location.protocol === 'https:' ? 'wss'+"://"+ location.host +"/api/coedit/websocket/luckysheet" : 'ws'+"://"+ location.host +"/api/coedit/websocket/luckysheet";
        ...
        async submitDatas(){
            ...
            if(msgMap && Object.keys(msgMap).length > 0)
            {
                ...
                var that = this;
                let bassicMd5 = md5(JSON.stringify(this.basicRowDatas));//旧数据md5值
                let rowMd5 = md5(JSON.stringify(this.rowDatas));//新数据md5值
                ...
                {//说明值未发生变化，直接返回
                    ...
                    for(var key in this.rowDatas) {
                        ...
                        {
                            bassicMd5 = md5(JSON.stringify(this.basicRowDatas[key]));//旧数据md5值
                            rowMd5 = md5(JSON.stringify(this.rowDatas[key]));//旧数据md5值
                            if(bassicMd5 != rowMd5){
