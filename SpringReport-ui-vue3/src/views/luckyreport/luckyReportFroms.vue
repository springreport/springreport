<template>
    <div class="contentbox" v-loading="loading" :element-loading-text="loadingText"  style="height:100vh;display: flex;flex-direction: column;overflow:hidden">
                ...
                <el-form :inline="true" class="demo-form-inline" :model="cellForm" ref="reportCellForm" v-show="tabIndex==1">
                    ...
                    <el-form-item label="是否允许修改" size="small">
                      ...
                      <div class="right-dataset-title" v-if="cellForm.otherCellCompare">
                        ...
                        <el-button class="addBtn" @click="addCompareCells" :disabled="attrDisabled">添加<icon-plus theme="outline" size="16" fill="#FFF" class="el-icon--right"/></el-button>
                     </div>
                     <el-collapse v-if="cellForm.otherCellCompare && cellForm.compareCells && cellForm.compareCells.length > 0">
                        <el-collapse-item v-for="(o,index) in cellForm.compareCells" :key="index">
                            <template #title>
                                比较单元格{{index+1}}
                                 <el-button class="right-block-el-icon-edit" title="编辑" type="primary" icon="icon-edit" circle size="small" @click.stop="editCompareCell(o,index)" :disabled="attrDisabled"></el-button>
                                 <el-button class="right-el-icon-delete" type="danger" title="删除" icon="icon-delete" circle size="small" @click.stop="deleteCompareCell(index)" :disabled="attrDisabled"></el-button>
                            </template>
                            <p class="column-tag" :title="o.sheetName" style="min-width:220px;max-width:220px;margin:0">sheet名称：{{o.sheetName}}</p>
                            <p class="column-tag" style="min-width:220px;max-width:220px;margin:0">坐标：{{o.coordinate}}</p>
                            <p class="column-tag" style="min-width:220px;max-width:220px;margin:0">单元格类型：{{commonUtil.getDictionaryValueName('cellType',o.cellType)}}</p>
                            <p class="column-tag" style="min-width:220px;max-width:220px;margin:0">比较类型：{{o.compareType}}</p>
                        </el-collapse-item>
                    </el-collapse>
        ...
        <el-dialog title="数据集" v-model="addDatasetsDialogVisiable" width="80%"  top="20px" :close-on-click-modal='false' @close='closeAddDataSet'>
              <el-tabs type="border-card">
                  ...
                  <el-tab-pane label="参数配置" >
                      ...
                      <el-form :inline="true" :model="paramForm" class="demo-form-inline" ref="paramRef">
                          ...
                          <el-form-item v-if="paramForm.paramType == 'select' || paramForm.paramType == 'mutiselect'  || paramForm.paramType == 'treeSelect' || paramForm.paramType == 'multiTreeSelect'" label="下拉选择内容" prop="selectContent" key="selectContent" :rules="filter_rules('下拉选择内容',{required:true})">
            ...
            <el-dialog  title="填报属性" v-model="datasourceDialog" width="50%" :modal="false"  :close-on-click-modal='false' @close='closeDatasourceDialog'  modal-class="hisDialog">
                <div class="table-box">
                    <div class="table-box-left">
                        ...
                        <div :class="o.isActive?'dataset-attr-active':'dataset-attr'" style="position:relative" >
                            ...
                            <icon-edit class="el-icon-edit" @click="editDatasourceAttr(o)"></icon-edit>
                            <icon-delete class="el-icon-delete" @click="deleteDatasourceAttr(index)"></icon-delete>
                        </div>
                    </div>
                    </div>
                    <div class="table-box-right">
                        ...
                        <el-table :data="datasourceAttr.tableDatas" border style="width: 100%" align="center" size="small" :header-cell-style="{background:'#eef1f6',color:'#606266'}">
                        ...
                        <el-table-column label="操作" align="center">
                            <template #default="scope">
                                <el-button @click="deleteDatasourceColumn(scope.$index)" type="primary" size="small">删除</el-button>
                            </template>
                        </el-table-column>
                        </el-table><br>
                        <el-button type="primary" size="small" icon="el-icon-plus" @click="addDatasourceKey">添加主键</el-button>
                        <el-table :data="datasourceAttr.keys" border style="width: 100%" align="center" size="small" :header-cell-style="{background:'#eef1f6',color:'#606266'}">
                        ...
                        <el-table-column label="操作" align="center">
                            <template #default="scope">
                                <el-button @click="deleteDatasourceKey(scope.$index)" type="primary" size="small">删除</el-button>
                            </template>
                        </el-table-column>
                        </el-table>
                        </div>
            ...
            <el-dialog title="添加数据源绑定" v-model="datasourceAttrDialog" width="50%"  :close-on-click-modal='false' @close='closeDatasourceAttr'>
            ...
            <el-dialog  title="添加属性关联" v-model="datasourceColumnDialog" width="50%"  :close-on-click-modal='false' @close='closeDatasourceColumn'>
            ...
            <el-dialog  title="添加主键" v-model="datasourceKeyDialog" width="50%"  :close-on-click-modal='false' @close='closeDatasourceKey'>
            ...
            <el-dialog title="单元格比较" v-model="cellCompareVisiable" width="50%" :close-on-click-modal='false' @close='closeCompareCells'>
