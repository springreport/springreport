...
export default {
  ...
  methods: {
    ...
    insertRows(coordinate, count, direction, sheetIndex) {
      ...
      if (
        this.sheetDatasource &&
        this.sheetDatasource[sheetIndex] &&
        this.sheetDatasource[sheetIndex].length > 0
      ) {
    ...
    getTableColumns() {
      ...
      if (columns) {
        ...
        var obj = {
          params: {
            datasourceId: this.datasourceAttr.datasourceId,
            tplSql: 'select * from ' + this.datasourceAttr.table,
            sqlType: 1,
          },
          url: this.apis.reportDesign.execSqlApi,
        };
      ...
      this.$refs['compareRef'].validate((valid) => {
        if (valid) {
          if (this.cellCompareForm.index != null) {
            this.cellForm.compareCells[this.cellCompareForm.index].sheetName =
              this.cellCompareForm.sheetName;
            this.cellForm.compareCells[this.cellCompareForm.index].coordinate =
              this.cellCompareForm.coordinate;
            this.cellForm.compareCells[this.cellCompareForm.index].cellType =
              this.cellCompareForm.cellType;
            this.cellForm.compareCells[this.cellCompareForm.index].compareType =
              this.cellCompareForm.compareType;
          } else {
            var obj = {
    ...
    changeReportAttr(data) {
      ...
      } else if ('datasourceChanged' == k) {
        let sheetName = v.sheetName;
        this.sheetDatasource[sheetIndex] = value;
        if (sheetIndex == currentIndex) {
          this.commonUtil.showMessage({
            message: '【' + sheetName + '】绑定数据源属性发生变更，操作人：' + data.userName,
            type: this.commonConstants.messageType.warning,
          });
