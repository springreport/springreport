...
export default {
      ...
      settingModalForm: [
        {
          ...
          prop: 'pageType',
          rules: { required: true },
          options: this.selectUtil.pageType,
        },
        {
          type: 'Select',
          label: '纸张布局',
          prop: 'pageLayout',
          rules: { required: true },
          options: this.selectUtil.pageLayout,
        },
        {
          type: 'Select',
          label: '页眉是否显示',
          ...
          change: this.changePageHeaderShow,
        },
        {
          type: 'Input',
          label: '页眉显示内容',
          prop: 'pageHeaderContent',
          rules: { required: true, maxLength: 100 },
        },
        {
          type: 'Select',
          label: '页眉显示位置',
          prop: 'pageHeaderPosition',
          rules: { required: true },
          options: this.selectUtil.pdfPosition,
        },
        {
          type: 'Select',
          label: '水印是否显示',
          ...
          rules: { required: true },
          options: this.selectUtil.yesNo,
          change: this.changeWaterMarkShow,
        },
        {
          type: 'Select',
          label: '水印类型',
          prop: 'waterMarkType',
          rules: { required: true },
          options: this.selectUtil.waterMarkType,
          change: this.changeWaterMarkType,
        },
        {
          type: 'Input',
          label: '水印内容',
        ...
        },
        {
          type: 'Input',
          label: '水印透明度',
          prop: 'waterMarkOpacity',
          rules: { required: true, type: 'number', min: 0.01, max: 0.99, float: 2 },
        },
        {
          type: 'Select',
          label: '页码是否显示',
        ...
        },
        {
          ...
          rules: { required: true },
          options: this.selectUtil.yesNo,
          change: this.changeHorizontalPage,
        },
  ...
  computed: {
    ...
    displayFields() {
      ...
      const fileds = this.datasetItemActive
        ? dataArr.find((item) => item.id === this.datasetItemActive)?.columns || []
        : [];
      return this.filedKeyword
        ? fileds.filter((item) => item.columnName.includes(this.filedKeyword))
        : fileds;
    },
    // 数据集id对应的名称
    datasetItem() {
  ...
  methods: {
    ...
    addOrEditGroup() {
      ...
      const reportTplId = this.$route.query.tplId; // reportTplId
      const obj = {
        url: this.groupForm.id
          ? this.apis.reportDesign.updateGroupApi
          : this.apis.reportDesign.insertGroupApi,
    ...
    endDraggable(datasetName, columnName) {
      ...
      if (!checkResult) {
        this.commonUtil.showMessage({
          message: '暂无编辑权限，如需编辑可联系创建者【' + this.creatorName + '】。',
        ...
        for (let index = 0; index < data.length; index++) {
          const element = data[index];
          {
            if (
              ...
              element.v.xxbt == 1 &&
              element.v.v &&
              element.v.v.split('|').length > 1 &&
              element.v.v.split('|').length < 5
            ) {
              let r = element.r;
              let c = element.c;
              let rs = 1;
    ...
    changeGroupSummary(type) {
      ...
      if (cells && cells.length > 0) {
        ...
        const sheetIndex = luckysheet.getSheet().index;
        luckysheet.sendServerSocketMsg('reportDesign', sheetIndex, obj, {
          k: type == 'r' ? 'groupSummaryDependencyr' : 'groupSummaryDependencyc',
        });
        this.saveTplCache();
      }
    },
    ...
    changeDigit() {
      ...
      if (cells && cells.length > 0) {
        for (let index = 0; index < cells.length; index++) {
          ...
          if (
            obj &&
            (obj.aggregateType == 'summary' ||
              obj.aggregateType == 'groupSummary' ||
              obj.unitTransfer)
          ) {
        ...
        for (var key in map) {
          ...
          if (dataFrom == 3) {
            ...
            if (groupSummaryDependencyc) {
              ...
              if (c1 >= coordinate) {
                value.groupSummaryDependencyc = this.commonUtil.getColumnNameFromC(
                  Number(c1) + count
                );
              }
            }
          ...
          if (dataFrom == 3) {
            ...
            if (groupSummaryDependencyr != null) {
              if (groupSummaryDependencyr >= start && groupSummaryDependencyr <= end) {
                ...
                value.groupSummaryDependencyr =
                  Number(groupSummaryDependencyr) - (end - start + 1) + 1;
              }
            }
          }
          if (r < start) {
          ...
          let obj = {
            ...
            params: {
              ...
              outParam: this.procedureOutParamTableData.tableData
                ...
                : '',
              sqlParams: this.paramTableData.tableData
    ...
    addParam() {
      this.$refs['paramRef'].validate((valid) => {
        if (valid) {
          ...
          let result = this.commonUtil.getItemIndexFromList(
            this.paramTableData.tableData,
            'paramCode',
          ...
          if (result.index >= 0) {
            ...
            this.paramTableData.tableData[result.index].paramRequired =
              this.paramForm.paramRequired;
            this.paramTableData.tableData[result.index].selectType = this.paramForm.selectType;
            this.paramTableData.tableData[result.index].selectContent =
              this.paramForm.selectContent;
            this.paramTableData.tableData[result.index].isRelyOnParams =
              this.paramForm.isRelyOnParams;
            ...
            this.paramTableData.tableData[result.index].checkStrictly =
              this.paramForm.checkStrictly;
            ...
            let row = {
              ...
              isRelyOnParams:
                this.paramForm.isRelyOnParams ? '2' : this.paramForm.isRelyOnParam,
    ...
    addInParam() {
      this.$refs['inParamRef'].validate((valid) => {
        if (valid) {
          //从列表冲根据paramCode获取是否已经添加该参数
          let result = this.commonUtil.getItemIndexFromList(
            this.procedureInParamTableData.tableData,
            'paramCode',
            this.procedureParamForm.paramCode
          );
          if (result.index >= 0) {
            //已经添加该参数，则修改参数内容
            this.procedureInParamTableData.tableData[result.index].paramName =
              this.procedureParamForm.paramName;
            this.procedureInParamTableData.tableData[result.index].paramType =
              this.procedureParamForm.paramType;
            this.procedureInParamTableData.tableData[result.index].paramDefault =
              this.procedureParamForm.paramDefault;
            this.procedureInParamTableData.tableData[result.index].paramRequired =
              this.procedureParamForm.paramRequired;
            this.procedureInParamTableData.tableData[result.index].paramHidden =
              this.procedureParamForm.paramHidden;
            this.procedureInParamTableData.tableData[result.index].dateFormat =
              this.procedureParamForm.dateFormat;
            this.procedureInParamTableData.tableData[result.index].componentType =
              this.procedureParamForm.componentType;
            this.procedureInParamTableData.tableData[result.index].selectType =
              this.procedureParamForm.selectType;
            this.procedureInParamTableData.tableData[result.index].selectContent =
              this.procedureParamForm.selectContent;
            this.procedureInParamTableData.tableData[result.index].checkStrictly =
              this.procedureParamForm.checkStrictly;
            this.procedureInParamTableData.tableData[result.index].datasourceId =
              this.procedureParamForm.datasourceId;
            this.procedureInParamTableData.tableData[result.index].isRelyOnParams =
              this.procedureParamForm.isRelyOnParams;
            this.procedureInParamTableData.tableData[result.index].paramRequired =
              this.procedureParamForm.paramRequired;
          } else {
            //未添加该参数，则列表中新增一条数据
    ...
    addOutParam() {
      this.$refs['outParamRef'].validate((valid) => {
        if (valid) {
          //从列表冲根据paramCode获取是否已经添加该参数
          let result = this.commonUtil.getItemIndexFromList(
            this.procedureOutParamTableData.tableData,
            'paramCode',
            this.procedureOutParamForm.paramCode
          );
          if (result.index >= 0) {
            //已经添加该参数，则修改参数内容
            this.procedureOutParamTableData.tableData[result.index].paramName =
              this.procedureOutParamForm.paramName;
            this.procedureOutParamTableData.tableData[result.index].paramType =
              this.procedureOutParamForm.paramType;
            this.procedureOutParamTableData.tableData[result.index].paramDefault =
              this.procedureOutParamForm.paramDefault;
          } else {
            //未添加该参数，则列表中新增一条数据
            let row = {
          ...
          this.commonUtil.doPost(obj).then((response) => {
            if (response.code == '200') {
              ...
              luckysheet.sendServerSocketMsg(
                'reportDesign',
                sheetIndex,
                { datasetName: obj.params.datasetName },
                { k: 'datasetChanged' }
              );
            }
          });
      ...
      if (luckysheetfiles) {
        ...
        for (let index = 0; index < luckysheetfiles.length; index++) {
          ...
          if (!luckysheetfile.isPivotTable) {
            ...
            var extraCustomCellConfigs = this.getSheetExtraCustomCellConfigs(
              cellDatas,
              luckysheetfile.index
            );
            ...
            var dataVerificationCells = this.getEmptyDataVerificationCells(
              configs.dataVerification,
              cellDatas
            );
            ...
            if (checkResult) {
              this.loading = false;
              this.commonUtil.showMessage({
                message: '图表左上角对应的单元格已被使用，请重新设置图表的位置。',
                type: 'error',
              });
              return;
            }
    ...
    getBlockInfos(blockData) {
      ...
      var startx = startCell.replace(/[^0-9]/gi, '') - 1;
      var starty = this.columnStringTonum(startCell.replace(startx + 1, '')) - 1;
      var endx = endCell.replace(/[^0-9]/gi, '') - 1;
    ...
    loadExcel(evt) {
      ...
      if (this.uploadType == 'xlsx') {
        ...
        if (suffix != 'csv') {
          this.commonUtil.showMessage({
            message: this.commonUtil.getMessageFromList('error.upload.filetype', ['csv']),
            type: this.commonConstants.messageType.error,
          });
          return;
        }
        ...
        this.commonUtil.showMessage({
          message: this.commonUtil.getMessageFromList('error.upload.filetype', ['xlsx/csv']),
          type: this.commonConstants.messageType.error,
        });
        return;
      }
      ...
      try {
        Axios.post(that.apis.reportTpl.uploadReportTplApi, formData, config).then((res) => {
          if (res.data.code == '200') {
            ...
            if (sameName) {
              that.commonUtil.showMessage({
                message: 'sheet名称【' + sameName + '】已经存在。',
                type: 'error',
              });
            }
          } else {
      ...
      this.$refs['conditionRef'].validate((valid) => {
        if (valid) {
          if (this.cellConditionForm.index != null) {
            //修改属性
            this.cellForm.cellconditions[this.cellConditionForm.index].property =
              this.cellConditionForm.property;
            this.cellForm.cellconditions[this.cellConditionForm.index].operator =
              this.cellConditionForm.operator;
            this.cellForm.cellconditions[this.cellConditionForm.index].type =
              this.cellConditionForm.type;
            this.cellForm.cellconditions[this.cellConditionForm.index].value =
              this.cellConditionForm.value;
            this.cellForm.cellconditions[this.cellConditionForm.index].dateFormat =
              this.cellConditionForm.dateFormat;
          } else {
            var data = {
          ...
          if (that.xAxisForm.dataType == 1) {
            try {
              ...
              if (!(test instanceof Array)) {
                this.commonUtil.showMessage({
                  message: "x轴数据请输入数组格式的数据，例如['a','b','c']",
                  type: 'error',
                });
                return;
              }
    ...
    luckysheetDeleteCellAfter(type, str, edr, stc, edc, sheetIndex, isWebsocket) {
      ...
      this.$refs['hiddenConditionRef'].validate((valid) => {
        if (valid) {
          if (that.cellHiddenConditionForm.index != null) {
            //修改属性
            that.cellForm.cellHiddenConditions[that.cellHiddenConditionForm.index].propertyName =
              that.cellHiddenConditionForm.propertyName;
            that.cellForm.cellHiddenConditions[that.cellHiddenConditionForm.index].property =
              that.cellHiddenConditionForm.property;
            that.cellForm.cellHiddenConditions[that.cellHiddenConditionForm.index].operator =
              that.cellHiddenConditionForm.operator;
            that.cellForm.cellHiddenConditions[that.cellHiddenConditionForm.index].type =
              that.cellHiddenConditionForm.type;
            that.cellForm.cellHiddenConditions[that.cellHiddenConditionForm.index].value =
              that.cellHiddenConditionForm.value;
            that.cellForm.cellHiddenConditions[that.cellHiddenConditionForm.index].dateFormat =
              that.cellHiddenConditionForm.dateFormat;
          } else {
            var data = {
    ...
    saveNewSheetCache(sheet) {
      ...
      if (configs) {
        ...
        luckysheet.sendServerSocketMsg('reportTplNewTempCache', gridKey + '-' + sheetIndex, configs, {
          k: 'viewReportCache',
        });
      }
    },
    saveTplAttrCache() {
      const reportTplId = this.$route.query.tplId;
      var gridKey = 'designMode-' + reportTplId;
      luckysheet.sendServerSocketMsg('reportTplAttrTempCache', gridKey, this.isParamMerge, {
        k: 'isParamMerge',
      });
    },
    ...
    frozenCancelAfter() {
      ...
      if (configs) {
        ...
        var gridKey = 'designMode-' + reportTplId;
        const sheetIndex = luckysheet.getSheet().index;
        luckysheet.sendServerSocketMsg('frozenCancleTempCache', gridKey + '-' + sheetIndex, configs, {
          k: 'viewReportCache',
        });
      }
    },
      ...
      this.$refs['subtotalRef'].validate((valid) => {
        if (valid) {
          if (that.cellSubTotalForm.index != null) {
            that.cellForm.subTotalCells[that.cellSubTotalForm.index].coords =
              that.cellSubTotalForm.coords;
            that.cellForm.subTotalCells[that.cellSubTotalForm.index].type =
      ...
      this.$refs['subtotalCalcRef'].validate((valid) => {
        if (valid) {
          if (that.subTotalCalcForm.index != null) {
            that.cellForm.subTotalCalc[that.subTotalCalcForm.index].attrs =
              that.subTotalCalcForm.attrs;
          } else {
            var data = {
      ...
      this.$refs['subtotalAttrsRef'].validate((valid) => {
        if (valid) {
          if (that.subTotalAttrsForm.index != null) {
            that.cellForm.subTotalAttrs[that.subTotalAttrsForm.index].name =
              that.subTotalAttrsForm.name;
            that.cellForm.subTotalAttrs[that.subTotalAttrsForm.index].fontColor =
              that.subTotalAttrsForm.fontColor;
            that.cellForm.subTotalAttrs[that.subTotalAttrsForm.index].fontSize =
              that.subTotalAttrsForm.fontSize;
            that.cellForm.subTotalAttrs[that.subTotalAttrsForm.index].bgColor =
              that.subTotalAttrsForm.bgColor;
            that.cellForm.subTotalAttrs[that.subTotalAttrsForm.index].fontWeight =
              that.subTotalAttrsForm.fontWeight;
          } else {
            var data = {
    ...
    addAuthClick() {
      ...
      if (!rangeAxis || rangeAxis.length == 0) {
        ...
        this.commonUtil.showMessage({
          message: '不能对多个选区执行此操作，请选择单个区域进行操作。',
          type: this.commonConstants.messageType.error,
        });
        return;
      }
      if (!this.isCreator) {
        this.commonUtil.showMessage({
          message: '您没有权限进行此操作，只有创建者【' + this.creatorName + '】才有权限进行操作。',
          type: this.commonConstants.messageType.error,
        });
        return;
      }
    ...
    pasteHandlerBefore(range, data) {
      if (this.isCreator) {
        ...
        if (!checkResult) {
          this.commonUtil.showMessage({
            message: this.commonUtil.getMessageFromList('error.auth.operate', [this.creatorName]),
            type: this.commonConstants.messageType.error,
          });
          return false;
        }
    ...
    formulaAuthCheck(range) {
      if (this.isCreator) {
        ...
        if (!checkResult) {
          this.commonUtil.showMessage({
            message:
              '公式计算结果不在可操作范围内，如需编辑可联系创建者【' + this.creatorName + '】。',
            type: this.commonConstants.messageType.error,
          });
        }
        return checkResult;
    ...
    showAuthInfoMsg() {
      if (!this.isCreator) {
        ...
        if (this.sheetRangeAuth) {
          if (this.sheetRangeAuth[sheetIndex]) {
            this.commonUtil.showMessage({
              message: this.commonUtil.getMessageFromList('warning.auth.operate', [
                this.creatorName,
              ]),
              type: this.commonConstants.messageType.warning,
            });
            ...
            this.commonUtil.showMessage({
              message: this.commonUtil.getMessageFromList('warning.auth.nooperate', [
                this.creatorName,
              ]),
              type: this.commonConstants.messageType.warning,
            });
    ...
    getTableColumns() {
      ...
      if (columns) {
        ...
        var obj = {
          params: {
            datasourceId: this.sqlForm.datasourceId,
            tplSql: 'select * from ' + this.datasourceTableName,
            sqlType: 1,
          },
          url: this.apis.reportDesign.execSqlApi,
        };
    ...
    getWhereByParam(row) {
      ...
      if (
        ...
        row.paramType == 'select' ||
        row.paramType == 'treeSelect'
      ) {
        ...
        text =
          '<if test="' + row.paramCode + '!=null and ' + row.paramCode + '.size() > 0' + '">\n';
