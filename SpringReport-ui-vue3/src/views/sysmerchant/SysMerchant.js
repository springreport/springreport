    ...
    return{
      pageData:{
        ...
        searchHandle:[
          ...
          {label:'重置',type:'warning',handle:()=>this.resetSearch(),auth:'sysMerchant_search'}
        ],
        //查询表单按钮end
        //表格数据start
        tableData:[],
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'sysMerchant_insert'},
          {label:'批量删除',type:'danger',handle:()=>this.deleteBatch(),auth:'sysMerchant_delete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        tableCols:[
          ...
          {label:'状态',prop:'status',align:'center',codeType:'status',formatter:this.commonUtil.getTableCodeName},
					...
					{label:'操作',prop:'operation',align:'center',type:'button',btnList:[
						{label:'查看',type:'primary',auth:'sysMerchant_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'primary',auth:'sysMerchant_edit',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
						{label:'删除',type:'primary',auth:'sysMerchant_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
