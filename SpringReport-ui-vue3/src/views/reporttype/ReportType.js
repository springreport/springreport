export default {
  ...
  data() {
    return{
      pageData:{
        ...
        //查询表单按钮start
        searchHandle:[
          {label:'查询',type:'primary',handle:()=>this.searchtablelist(),auth:'reportType_search'},
        ...
        tableCols:[
					...
					{label:'操作',prop:'operation',align:'center',type:'button',width:200,fixed:'right',btnList:[
						{label:'查看',type:'primary',auth:'reportType_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'primary',auth:'reportType_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
						{label:'删除',type:'primary',auth:'reportType_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
        ...
        //modal表单 start
        modalForm:[
					{type:'Input',label:'类型名称',prop:'reportTypeName',rules:{required:true,maxLength:50}},
        ],
        //modal表单 end
        //modal 数据 start
        modalData : {//modal页面数据
  ...
  methods:{
    ...
    searchtablelist(){
      var obj = {
        url:this.apis.reportType.listApi,
    ...
    getDetail(id){
      var obj = {
        url:this.apis.reportType.getDetailApi,
        params:{id:id},
      }
    ...
    deleteBatch(){
        ...
        if(length == 0)
        {
          ...
          let obj = {
            url:this.apis.reportType.deleteBatchApi,
            messageContent:this.commonUtil.getMessageFromList("confirm.delete",null),
            callback:this.searchtablelist,
            params:ids,
            type:"post",
