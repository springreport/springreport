  ...
  data() {
    return {
      pageData: {
        ...
        // 表格工具栏按钮 start
        tableHandles: [
          {label:'新建目录',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert,null,'1'),auth:'screenTpl_folder'},
          {label:'新建文档',type:'warning',handle:()=>this.showModal(this.commonConstants.modalType.insert,null,'2'),auth:'screenTpl_insert'},
          {label:'刷新',type:'danger',handle:()=>this.searchtablelist(),auth:'screenTpl_search'},
        ],
        // 表格工具栏按钮 end
        selectList: [], // 表格选中的数据
        // 表格分页信息start
        ...
        tableCols: [
          ...
          { label: '操作', prop: 'operation', align: 'center', type: 'button',width:400,fixed:'right', btnList: [
            { label: '查看', type: 'primary', auth: 'screenTpl_getDetail', handle: (row) => this.showModal(this.commonConstants.modalType.detail, row.id),show:(row)=>this.isShowBtn(row)},
            { label: '编辑', type: 'primary', auth: 'screenTpl_update', handle: (row) => this.showModal(this.commonConstants.modalType.update, row.id,row.type) },
            { label: '大屏设计', type: 'primary', auth: 'screenTpl_screenDesign', handle: (row) => this.screenDesign(row),show:(row)=>this.isShowBtn(row)},
            { label: '查看大屏', type: 'primary', auth: 'screenTpl_viewScreen', handle: (row) => this.screenView(row),show:(row)=>this.isShowBtn(row)},
            { label: '删除', type: 'primary', auth: 'screenTpl_delete', handle: (row) => this.deleteOne(row.id,row.type) }
          ] }
        ],
        // 表格列表头end
        // modal配置 start
