...
export default {
    ...
    return {
      ...
      pageData: {
        ...
        tableHandles: [
          ...
          { label: '导出模板', type: 'warning', position: 'right', iconClass: 'action-icon-download', handle: () => this.exportTemplate(), auth: 'screen_export' },
          { label: '导入模板', type: 'danger', position: 'right', iconClass: 'action-icon-upload', handle: () => this.importTemplate(), auth: 'screen_import' },
        ],
        // 表格工具栏按钮 end
        selectList: [], // 表格选中的数据
        // 表格分页信息start
        ...
        tableCols: [
          {
            ...
            btnList: [
              ...
              },
              {
                label: '大屏设计',
                type: 'primary',
                auth: 'screenTpl_screenDesign',
                handle: (row) => this.screenDesign(row),
                show: (row) => this.isShowBtn(row),
              },
              {
                label: '查看大屏',
                type: 'primary',
                auth: 'screenTpl_viewScreen',
                handle: (row) => this.screenView(row),
                show: (row) => this.isShowBtn(row),
              },
  ...
  methods: {
    ...
    screenView(row) {
      const viewReport = this.$router.resolve({
        name: 'screenView',
        query: { tplId: row.id, sendRequest: 1 },
      });
      window.open(viewReport.href, '_blank');
    },
    copyTpl(row) {
