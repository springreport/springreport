...
{
    ElMessageBox.confirm(params.messageContent,
    {
...
commonUtil.changeMapType = async function(mapType,that){
    var features = null;
    let registerMap = that.$echarts.getMap(mapType);
    if(!registerMap)
    {
        await Axios.get("/geoJson/"+mapType+".json").then((res)=>{
            that.$echarts.registerMap(mapType,res.data);
            registerMap = res.data;
        });
    }
    if(registerMap.geoJSON && registerMap.geoJSON.features)
    {
        features = registerMap.geoJSON.features
    }else if(registerMap.features)
    {
        features = registerMap.features
    }
     ...
     for (let index = 0; index < features.length; index++) {
         ...
         if(element.properties.name)
         {
             var obj={
                ...
                value:element.properties.center[0]+"-"+element.properties.center[1]+"-"+element.properties.adcode+"-"+element.properties.name
            }
            areas.push(obj);
         }
...
commonUtil.mapDrill = async function(chartsComponents,component,data,sendRequest,that){
    if(data && data.datum && Object.keys(data.datum).length > 0){
        ...
        if(component.drillType == '2'){
            ...
            if(!sendRequest ){
                // chartsComponents[component.id].updateSpec(JSON.parse(JSON.stringify(component.spec)),true);
                commonUtil.reLoadChart(chartsComponents,component,sendRequest,that)
            } else{
