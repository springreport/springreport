...
<template>
  <div>
    ...
    <div class="right-dataset-title" v-if="component.type == 'tableMap'">
      ...
      <div class="right-dataset-warp">
        ...
        <el-form-item label="背景颜色">
          <input-color-picker
            :value="component.spec.background"
            @change="
              (val) => {
                component.spec.background = val;
        ...
        <div
          v-if="component.type.toLowerCase().indexOf('basicmap') < 0 && component.type.toLowerCase().indexOf('tablemap')<0"
          class="df-c"
          style="flex-wrap: wrap"
        >
          <el-form-item
            v-for="(item, index) in component.spec.color"
            :key="index"
            label=""
            class="color-el-form-item"
          >
            <el-color-picker
        ...
        >
          <el-form-item
            v-for="(item, index) in component.spec.color.range"
            :key="index"
            label=""
            class="color-el-form-item"
          >
        ...
        <div style="text-align: center">
          <el-link type="info" @click="clearColor(component)"
      ...
      <div v-if="component.type.toLowerCase().indexOf('wordcloud') < 0">
        ...
        <div class="right-dataset-warp">
          <el-form-item label="是否显示" class="df-form-item">
            <el-switch
              v-model="component.spec.title.visible"
              active-text="是"
              inactive-text="否"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          ...
          <el-form-item v-if="component.spec.title.visible" label="标题内容">
            <el-input
              v-model="component.spec.title.text"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item v-if="component.spec.title.visible" label="对齐方式">
            <el-select
              v-model="component.spec.title.align"
              placeholder="请选择"
              style="width: 180px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            >
          ...
          <el-form-item v-if="component.spec.title.visible" label="字体颜色">
            <input-color-picker
              :value="component.spec.title.textStyle.fill"
              @change="
                (val) => {
                  component.spec.title.textStyle.fill = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
      ...
      <div v-if="component.type.toLowerCase().indexOf('pie') >= 0">
        ...
        <div class="right-dataset-warp">
          <el-form-item label="外半径(0-1)">
            <el-input-number
              v-model="component.spec.outerRadius"
              style="width: 100%"
          ...
          <el-form-item label="内半径(0-1)">
            <el-input-number
              v-model="component.spec.innerRadius"
              style="width: 100%"
          ...
          <el-form-item label="扇区间隔">
            <el-input
              v-model="component.spec.padAngle"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="圆角">
            <el-input
              v-model="component.spec.pie.style.cornerRadius"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="轮播动画" class="df-form-item">
            <el-switch
              v-model="component.spec.isLoop"
              active-text="是"
              inactive-text="否"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
      ...
      <div v-if="component.type.toLowerCase().indexOf('radar') >= 0">
        ...
        <div class="right-dataset-warp">
          <el-form-item label="折线宽度">
            <el-input
              v-model="component.spec.line.style.lineWidth"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="区域显示" class="df-form-item">
            <el-switch
              v-model="component.spec.area.visible"
              active-text="是"
              inactive-text="否"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          ...
          <el-form-item label="圆形网格" class="df-form-item">
            <el-switch
              v-model="component.spec.axes[0].grid.smooth"
              active-text="是"
              inactive-text="否"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          ...
          <el-form-item label="坐标轴颜色">
            <input-color-picker
              :input-width="160"
              :value="component.spec.axes[0].grid.style.stroke"
              @change="
                (val) => {
                  component.spec.axes[0].grid.style.stroke = val;
          ...
          <el-form-item label="轴标签颜色">
            <input-color-picker
              :input-width="160"
              :value="component.spec.axes[1].label.style.fill"
              @change="
                (val) => {
                  component.spec.axes[1].label.style.fill = val;
      ...
      <div v-if="component.type.toLowerCase().indexOf('gauge') >= 0">
        ...
        <div class="right-dataset-warp">
          <el-form-item label="上边距">
            <el-input
              v-model.number="component.spec.padding.top"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="下边距">
            <el-input
              v-model.number="component.spec.padding.bottom"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="外半径(0-1)">
          ...
          <el-form-item label="起始角度">
            <el-input
              v-model="component.spec.startAngle"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="结束角度">
            <el-input
              v-model="component.spec.endAngle"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="仪表盘背景色">
            <input-color-picker
              :input-width="150"
              :value="component.spec.gauge.track.style.fill"
              @change="
                (val) => {
                  component.spec.gauge.track.style.fill = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
          ...
          <el-form-item label="最小值">
            <el-input
              v-model.number="component.spec.axes[0].min"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="最大值">
            <el-input
              v-model.number="component.spec.axes[0].max"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="标签字体颜色">
            <input-color-picker
              :input-width="140"
              :value="component.spec.axes[0].label.style.fill"
              @change="
                (val) => {
                  component.spec.axes[0].label.style.fill = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
          ...
          <el-form-item label="标签字体大小">
            <el-input
              v-model.number="component.spec.axes[0].label.style.fontSize"
              style="width: 140px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item
            v-if="component.type.toLowerCase().indexOf('seriesgauge') < 0"
            label="指标卡是否显示"
            class="df-form-item"
          >
            <el-switch
              v-model="component.spec.indicator.visible"
              active-text="是"
              inactive-text="否"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          ...
          <el-form-item
            v-if="
              component.type.toLowerCase().indexOf('seriesgauge') < 0 &&
              component.spec.indicator.visible
            "
            label="上边距"
          >
            <el-input
              v-model="component.spec.indicator.offsetY"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item
          ...
          >
            <el-input
              v-model.number="component.spec.indicator.content.style.fontSize"
              style="width: 140px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          ...
          >
            <input-color-picker
              :input-width="140"
              :value="component.spec.indicator.content.style.fill"
              @change="
                (val) => {
                  component.spec.indicator.content.style.fill = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
        ...
        <div class="right-dataset-title" v-if="component.type.toLowerCase().indexOf('seriesgauge')>=0">
          ...
          <div class="right-dataset-warp" v-if="component.type.toLowerCase().indexOf('seriesgauge')>=0">
            ...
            <el-form-item label="标签是否显示" class="df-form-item">
              <el-switch
                v-model="component.spec.gauge.label.visible"
                active-text="是"
                inactive-text="否"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              />
            ...
            <el-form-item v-if="component.spec.gauge.label.visible" label="标签位置">
              <el-select
                v-model="component.spec.gauge.label.position"
                placeholder="请选择"
                style="width: 180px"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              >
            ...
            <el-form-item v-if="component.spec.gauge.label.visible" label="标签颜色">
              <input-color-picker
                :value="component.spec.gauge.label.style.fill"
                @change="
                  (val) => {
                    component.spec.gauge.label.style.fill = val;
                    commonUtil.reLoadChart(chartsComponents, component);
                  }
                "
              />
            ...
            <el-form-item v-if="component.spec.gauge.label.visible" label="字体大小">
              <el-input
                v-model.number="component.spec.gauge.label.style.fontSize"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              />
            </el-form-item>
            <el-form-item v-if="component.spec.gauge.label.visible" label="标签格式">
              <el-input
                v-model="component.spec.gauge.label.formatter"
                style="width: 120px"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              >
              <template #suffix>
      ...
      <div v-if="component.type.toLowerCase().indexOf('circularprogress') >= 0">
        ...
        <div class="right-dataset-warp">
          ...
          <el-form-item v-if="component.spec.indicator.visible" label="指标卡字体大小">
            <el-input
              v-model.number="component.spec.indicator.title.style.fontSize"
              style="width: 140px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item v-if="component.spec.indicator.visible" label="指标卡字体颜色">
            <input-color-picker
              :input-width="140"
              :value="component.spec.indicator.title.style.fill"
              @change="
                (val) => {
                  component.spec.indicator.title.style.fill = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
          ...
          <el-form-item label="指标卡触发类型">
            <el-select
              v-model="component.spec.indicator.trigger"
              placeholder="请选择"
              style="width: 140px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            >
      ...
      <div v-if="component.type.toLowerCase().indexOf('barprogress') >= 0">
        ...
        <div class="right-dataset-warp">
          <el-form-item label="进度条宽度">
            <el-input
              v-model.number="component.spec.bandWidth"
              style="width: 140px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item label="圆角">
            <el-input
              v-model.number="component.spec.cornerRadius"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          ...
          <el-form-item label="y轴位置">
            <el-select
              v-model="component.spec.axes[0].orient"
              placeholder="请选择"
              style="width: 180px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            >
      ...
      <div v-if="component.type.toLowerCase().indexOf('liquid') >= 0">
        ...
        <div class="right-dataset-warp">
          <el-form-item label="形状">
            <el-select
              v-model="component.spec.maskShape"
              placeholder="请选择"
              style="width: 180px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            >
      ...
      <div v-if="component.type.toLowerCase().indexOf('basicmap') >= 0 || component.type.toLowerCase().indexOf('tablemap')>=0">
        ...
        <div class="right-dataset-warp">
          ...
          <el-form-item label="默认填充色">
            <input-color-picker
              :input-width="140"
              :value="component.spec.defaultFillColor"
              @change="
                (val) => {
                  component.spec.defaultFillColor = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
          ...
          <el-form-item label="标签是否显示" class="df-form-item">
            <el-switch
              v-model="component.spec.label.visible"
              active-text="是"
              inactive-text="否"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          ...
          <el-form-item v-if="component.spec.label.visible" label="标签颜色">
            <input-color-picker
              :value="component.spec.label.style.fill"
              @change="
                (val) => {
                  component.spec.label.style.fill = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
          ...
          <el-form-item v-if="component.spec.label.visible" label="字体大小">
            <el-input
              v-model.number="component.spec.label.style.fontSize"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item v-if="component.spec.label.visible" label="标签格式">
            <el-input
              v-model="component.spec.label.formatter"
              style="width: 120px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            >
            <template #suffix>
          ...
          <el-form-item label="">
            <div v-for="(value, key) in component.spec.nameMap" :key="key">
              <el-tag :title="key" style="width: 100px">{{
                key.length > 6 ? key.substring(0, 6) + '...' : key
              }}</el-tag>
              <b style="color: white">-></b>
              <el-input
                v-model="component.spec.nameMap[key]"
                style="width: 120px"
                size="small"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              />
      ...
      >
        ...
        <div class="right-dataset-warp">
          ...
          <el-form-item
            v-if="component.type.toLowerCase().indexOf('histogram') >= 0"
            :label="getColorLabel() + '宽度'"
          >
            <el-input
              v-model="component.spec.barWidth"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <el-form-item v-if="component.type.toLowerCase().indexOf('histogram') >= 0" label="圆角">
            <el-input
              v-model.number="component.spec.bar.style.cornerRadius"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          </el-form-item>
          <!--
          ...
          >
            <el-switch
              v-model="component.spec.lineLabel.visible"
              active-text="是"
              inactive-text="否"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            />
          ...
          <el-form-item
            v-if="
              (component.type.toLowerCase().indexOf('line') >= 0 ||
                component.type.toLowerCase().indexOf('area') >= 0) &&
              component.spec.lineLabel.visible
            "
            label="标签位置"
          >
            <el-select
              v-model="component.spec.lineLabel.position"
              placeholder="请选择"
              style="width: 180px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            >
          ...
          >
            <input-color-picker
              :value="component.spec.lineLabel.style.fill"
              @change="
                (val) => {
                  component.spec.lineLabel.style.fill = val;
                  commonUtil.reLoadChart(chartsComponents, component);
                }
              "
            />
      ...
      <div v-if="component.type.toLowerCase().indexOf('combocharthl')>=0">
        ...
        <div v-if="component.type.toLowerCase().indexOf('line')>=0 || component.type.toLowerCase().indexOf('area')>=0 || component.type.toLowerCase().indexOf('histogram')>=0 || component.type.toLowerCase().indexOf('combocharthl')>=0">
            ...
            <el-form-item label="开启自动旋转" class="df-form-item">
              <el-switch
                v-model="component.spec.axes[0].label.autoRotate"
                active-text="是"
                inactive-text="否"
            ...
            <el-form-item label="开启自动省略" class="df-form-item">
              <el-switch
                v-model="component.spec.axes[0].label.autoLimit"
                active-text="是"
                inactive-text="否"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              />
            ...
            <el-form-item label="开启自动省略" class="df-form-item">
              <el-switch
                v-model="component.spec.axes[1].label.autoLimit"
                active-text="是"
                inactive-text="否"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              />
      ...
      <div
        v-if="
          ...
          && component.type.toLowerCase().indexOf('combocharthl')<0
          && component.type.toLowerCase().indexOf('tablemap')<0
        "
      >
        <div class="right-dataset-title">
        ...
        <div class="right-dataset-warp">
          ...
          <el-form-item
            v-if="
              component.spec.label.visible && component.type.toLowerCase().indexOf('funnel') < 0
            "
            label="标签位置"
          >
            <el-select
            ...
            <el-select
              v-if="component.type.toLowerCase().indexOf('pie') >= 0"
              v-model="component.spec.label.position"
              placeholder="请选择"
              style="width: 180px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            ...
            <el-select
              v-if="component.type.toLowerCase().indexOf('scatter') >= 0"
              v-model="component.spec.label.position"
              placeholder="请选择"
              style="width: 180px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
            ...
            <el-select
              v-if="component.type.toLowerCase().indexOf('radar') >= 0"
              v-model="component.spec.label.position"
              placeholder="请选择"
              style="width: 180px"
              @change="commonUtil.reLoadChart(chartsComponents, component)"
          ...
          <div v-if="component.type.toLowerCase().indexOf('funnel') >= 0">
            ...
            <el-form-item label="是否显示" class="df-form-item">
              <el-switch
                v-model="component.spec.outerLabel.visible"
                active-text="是"
                inactive-text="否"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              />
            ...
            <el-form-item v-if="component.spec.outerLabel.visible" label="标签位置">
              <el-select
                v-model="component.spec.outerLabel.position"
                placeholder="请选择"
                style="width: 180px"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              >
            ...
            <el-form-item v-if="component.spec.outerLabel.visible" label="字体大小">
              <el-input
                v-model.number="component.spec.outerLabel.style.fontSize"
                @change="commonUtil.reLoadChart(chartsComponents, component)"
              />
            </el-form-item>
            <el-form-item v-if="component.spec.outerLabel.visible" label="字体颜色">
              <input-color-picker
                :value="component.spec.outerLabel.style.fill"
                @change="
                  (val) => {
                    component.spec.outerLabel.style.fill = val;
                    commonUtil.reLoadChart(chartsComponents, component);
                  }
                "
              />
      ...
      <div class="right-dataset-warp">
        <el-form-item label="提示框标题">
          <el-input
            v-model="component.spec.tooltip.mark.title.valueStr"
            style="width: 120px"
            @change="changeTooltipMarkTitle(chartsComponents, component)"
          />
        </el-form-item>
        <el-form-item>
          <template #label>
            <div class="df-c-b" v-if="component.type.toLowerCase().indexOf('basicmap')>=0">
              <span>提示框内容配置</span>
              <el-button type="primary" size="small" class="addBtn" @click="addTooltipContent(component)"
                ><icon-plus
                  theme="outline"
                  size="16"
                  fill="#FFF"
          ...
          <el-collapse v-if="component.spec.tooltip.mark.content && component.spec.tooltip.mark.content.length > 0 && component.type.toLowerCase().indexOf('basicmap')>=0" class="sub-collapse" style="width:100%">
            <el-collapse-item v-for="(item, index) in component.spec.tooltip.mark.content" :key="index">
              <template v-slot:title>
                内容配置{{ index + 1 }}
                <div class="right-block-el-icon-edit" @click.stop="editTooltipContent(item, index)" />
                <div class="right-el-icon-delete" @click.stop="deleteTooltipContent(index)" />
              </template>
    ...
    >
      <el-form class="demo-form-inline" label-position="top" ref="addColorRef">
    ...
    <el-dialog title="提示框配置" width="32%" v-model="addTooltipVisiable" :close-on-click-modal="false" @close="closeAddTooltip">
  ...
  export default {
    ...
    methods: {
      ...
      radiusMiuns(type) {
        if (type == 1) {
          //外半径
          this.component.spec.outerRadius = (Number(this.component.spec.outerRadius) - 0.1).toFixed(
            1
          );
        } else {
          //内半径
          this.component.spec.innerRadius = (Number(this.component.spec.innerRadius) - 0.1).toFixed(
            1
          );
        }
      ...
      changeSystemColor() {
        let colors = JSON.parse(JSON.stringify(this.screenConstants.systemChartColors[this.systemColor]));
        if (this.component.type.toLowerCase().indexOf('basicmap') >= 0 || this.component.type.toLowerCase().indexOf('tablemap') >= 0) {
