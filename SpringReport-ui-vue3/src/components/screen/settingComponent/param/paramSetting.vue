<template>
  <div>
    <div class="right-dataset-title df-c-b">
      ...
      <el-button class="addBtn" @click="showAddParamDailog"
        ><i class="el-icon-plus el-icon--left" />添加</el-button
      >
    </div>

    <el-collapse v-if="component.params && component.params.length > 0" class="sub-collapse">
      <el-collapse-item v-for="(item, index) in component.params" :key="index">
        <template v-slot:title>
          参数{{ index + 1 }}
          <div class="right-block-el-icon-edit" @click.stop="editParam(item, index)" />
          <div class="right-el-icon-delete" @click.stop="deleteParam(index)" />
        </template>
    ...
    >
      <div class="el-dialog-div">
        <el-form label-position="top" :model="paramForm" class="demo-form-inline" ref="paramRef">
          <el-form-item
            label="参数名称"
            prop="paramName"
          ...
          >
            <el-select v-model="paramForm.isRelyOnParams" placeholder="是否依赖其他参数" size="">
          ...
          >
            <el-select v-model="paramForm.checkStrictly" placeholder="选择父子联动" size="">
              <el-option label="是" value="1" />
              <el-option label="否" value="2" />
            </el-select>
          </el-form-item>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="closeDialog" size="">取 消</el-button>
          <el-button type="primary" @click="addParam" size="">确 定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
        ...
        this.$refs['paramRef'].validate((valid) => {
          if (valid) {
            ...
            if (this.component.params) {
              if (this.isEdit) {
                this.component.params[this.editIndex].paramName = this.paramForm.paramName;
                this.component.params[this.editIndex].paramCode = this.paramForm.paramCode;
                this.component.params[this.editIndex].paramType = this.paramForm.paramType;
                this.component.params[this.editIndex].paramDefault = this.paramForm.paramDefault;
                this.component.params[this.editIndex].paramRequired = this.paramForm.paramRequired;
                this.component.params[this.editIndex].paramHidden = this.paramForm.paramHidden;
                this.component.params[this.editIndex].selectContent = this.paramForm.selectContent;
                this.component.params[this.editIndex].selectType = this.paramForm.selectType;
                this.component.params[this.editIndex].width = this.paramForm.width;
                this.component.params[this.editIndex].datasourceId = this.paramForm.dataSourceId;
                ...
                this.component.params[this.editIndex].isRelyOnParams =
