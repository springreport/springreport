...
export default {
  data() {
    return {
      ...
      chartModalForm: [
        { type: 'Input', label: '图表标题', prop: 'chartName', rules: { required: true }},
  ...
  methods: {
    getDocTplSettings() {
      ...
      this.commonUtil.doPost(param).then(response => {
        if (response.code == '200') {
          ...
          const pagers = paperSizeDomOptionsDom.querySelectorAll('li')
          for (let index = 0; index < pagers.length; index++) {
            const element = pagers[index]
          ...
          // 设置纸张方向并回显
          const pageDirection = response.responseData.paperDirection
          ...
          const pagerDirections = paperDirectionDomOptionsDom.querySelectorAll('li')
          for (let index = 0; index < pagerDirections.length; index++) {
            const element = pagerDirections[index]
            element.classList.remove('active')
          ...
          // 设置水印
          const watermark = response.responseData.watermark
          if (watermark) {
    ...
    saveTpl() {
      ...
      const tplSettings = this.instance.command.getValue()
      const options = this.instance.command.getOptions()
      const paperDirection = options.paperDirection
      const paperMargin = this.instance.command.getPaperMargin()
      var obj = {
    ...
    getDatasetColumns(element, type,isEdit) {
      ...
      this.commonUtil.doPost(obj).then(response => {
        ...
        if (type == '1') {
          ...
          that.$refs['codeModalRef'].$forceUpdate()
        }
        if(isEdit){
          this.sqlColumnTableData.tableData = response.responseData
        }
