  ...
  data() {
    return {
      ...
      pageData: {
        ...
        // 查询表单按钮start
        searchHandle: [
          { label: '查询', type: 'primary', handle: () => this.searchtablelist(), auth: 'screenTemplate_search' },
          { label: '重置', type: '', handle: () => this.resetSearch(), auth: 'screenTemplate_search' }
        ],
        // 查询表单按钮end
        // 表格数据start
        lazy: true,
        ...
        tableHandles: [
          ...
          { label: '新建分类', type: 'primary', position: 'right', iconClass: 'action-icon-add', handle: () => this.showModal(this.commonConstants.modalType.insert, null, '1'), auth: 'screenTemplate_folder' },
          { label: '新建模板', type: 'info', position: 'right', iconClass: 'action-icon-add', handle: () => this.showModal(this.commonConstants.modalType.insert, null, '2'), auth: 'screenTemplate_insert' },
          { label: '刷新', type: 'danger', position: 'left', iconClass: 'action-icon-refresh', handle: () => this.searchtablelist(), auth: 'screenTemplate_search' }
        ],
        // 表格工具栏按钮 end
        selectList: [], // 表格选中的数据
        // 表格分页信息start
        ...
        tableCols: [
          { label: '操作', prop: 'operation', align: 'center', type: 'dropdown', width: 54, btnList: [
            { label: '查看', type: 'text', auth: 'screenTemplate_getDetail', handle: (row) => this.showModal(this.commonConstants.modalType.detail, row.id), show: (row) => this.isShowBtn(row) },
            { label: '编辑', type: 'text', auth: 'screenTemplate_edit', handle: (row) => this.showModal(this.commonConstants.modalType.update, row.id, row.type) },
            { label: '大屏设计', type: 'text', auth: 'screenTemplate_design', handle: (row) => this.screenDesign(row), show: (row) => this.isShowBtn(row) },
            { label: '查看大屏', type: 'text', auth: 'screenTemplate_view', handle: (row) => this.screenView(row), show: (row) => this.isShowBtn(row) },
            { label: '删除', type: 'text', auth: 'screenTemplate_delete', handle: (row) => this.deleteOne(row.id, row.type) }
          ] },
        ...
        copyModalForm: [
          ...
          { type: 'Input', label: '大屏宽度', prop: 'width', rules: { required: true, type: 'number' }},
          { type: 'Input', label: '大屏高度', prop: 'height', rules: { required: true, type: 'number' }},
          { type: 'Select', label: '大屏数据源', prop: 'dataSource', disabled: this.commonUtil.disabled, rules: { required: false }, multiple: true, props: { label: 'name', value: 'id' }}
        ],
        // modal表单 end
        // modal 数据 start
        ...
        // modal表单 start
        folderModalForm: [
          { type: 'Input', label: '目录名称', prop: 'fieldName', rules: { required: true, maxLength: 50 }}
        ],
        // modal表单 end
        // modal 数据 start
