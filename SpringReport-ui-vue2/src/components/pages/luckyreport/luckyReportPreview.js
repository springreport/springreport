...
export default {
  ...
  mounted() {
    ...
    $(document).off('click').click(function(event) {
      if (event.target.className.indexOf('icon-saveas24') >= 0) { // 另存为协同编辑文档
        that.modalConfig.show = true
      } else if (event.target.className.indexOf('icon-baocun2') >= 0) { // 保存
        that.submitDatas()
      } else if (event.target.className.indexOf('icon-lishijilu') >= 0) { // 上报历史记录
        that.reportHisDialog = true
        that.searchtablelist()
      } else if (event.target.className.indexOf('icon-8tupian-1') >= 0) { // 上传图片
        if (that.ctx) {
  ...
  methods: {
    ...
    getReportParam() {
      ...
      this.commonUtil.doPost(obj, headers).then(response => {
        if (response.code == '200') {
          const result = response.responseData.params
              ...
              if (result[i].params[m].paramType == 'mutiselect' || result[i].params[m].paramType == 'multiTreeSelect' || result[i].params[m].componentType == 'mutiselect' || result[i].params[m].componentType == 'multiTreeSelect') {
                ...
                if (that.isDrillBack == 1) {
                  ...
                  if (result[i].params[m].paramType == 'select') {
                    const relyOnParams = result[i].params[m].relyOnParams
                    const isRelyOnParams = result[i].params[m].isRelyOnParams
          ...
          if (isPagination) {
            this.searchHandle.push({
              btnType: 'dropDown', label: '定向导出', icon: 'action-icon action-icon-conditional-export',
              downs: [
            ...
            this.searchHandle.push({
              btnType: 'dropDown', label: '本页导出', icon: 'action-icon action-icon-current-export',
              downs: [
    ...
    setPagination(patination, isParamMerge, mergePagination) {
      if (isParamMerge == '1') {
        const element = this.searchData.params[0]
    ...
    transOnline() {
      ...
      this.commonUtil.doPost(obj).then(response => {
        if (response.code == '200') {
          const id = response.responseData.id
          const viewReport = this.$router.resolve({ name: 'onlineReport', query: { tplId: id }})
    ...
    back() {
      ...
      if (!this.drillRelations[this.currentTplId]) { // 判断是否还有上级报表
        this.isDrill = 2
      }
      this.getReportParam()
    },
      ...
      for (let index = 0; index < luckysheetfiles.length; index++) {
        ...
        if (charts && charts.length > 0) {
          for (let t = 0; t < charts.length; t++) {
            ...
            const baseCanvas = document.getElementById(chart_id_c).getElementsByTagName('canvas')[0]
    ...
    rangeSelect(sheet, range) {
      ...
      if (this.extendCellOrigins[sheetIndex]) {
        const cells = this.getSelectRangeCells(range)
        for (let index = 0; index < cells.length; index++) {
          const cell = cells[index]
          const r = cell[0]
          const c = cell[1]
          if (this.extendCellOrigins[sheetIndex][r + '_' + c]) {
            const cellValue = luckysheet.getCellValue(r, c, { type: 'v' })
    ...
    dataVerify(r, c, originR, originC, v, sheetIndex, wrongMsg, rowFlag, cellExtend) {
      ...
      if (this.extraCustomCellConfigs[sheetIndex]) {
        ...
        if (cellConfig) {
          ...
          if (cellConfig.otherCellCompare && cellConfig.compareCells != null && cellConfig.compareCells.length > 0) { // 与其他单元格比较
      ...
      if (cellConfig.lengthValid) {
        ...
        if (cellConfig.textValidRule) {
          switch (cellConfig.textValidRule) {
            ...
            case '99':
              if (cellConfig.regex) {
                const regex = eval(cellConfig.regex)
                var flag = this.commonUtil.regexValid(v, regex)
                if (!flag) {
