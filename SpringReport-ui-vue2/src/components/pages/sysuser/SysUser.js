export default {
  ...
  data() {
    return {
      pageData: {
        ...
        // 查询表单按钮start
        searchHandle: [
          { label: '清除条件', type: '', handle: () => this.resetSearch(), auth: 'sysUser_search' },
        ...
        // 表格工具栏按钮 start
        tableHandles: [
          { label: '新增', type: 'primary', position: 'right', iconClass: 'action-icon-add', handle: () => this.showModal(this.commonConstants.modalType.insert), auth: 'sysUser_insert' },
          { label: '批量删除', type: 'danger', position: 'left', iconClass: 'action-icon-del', handle: () => this.deleteBatch(), auth: 'sysUser_batchDelete' }
        ],
        // 表格工具栏按钮 end
        selectList: [], // 表格选中的数据
        // 表格分页信息start
        ...
        modalForm: [
          ...
          { type: 'TreeSelect', label: '所属部门', prop: 'deptId', rules: { required: true }, lazy: false, clearable: true, ref: 'treeSelect', data: [], valueProp: 'deptId', props: { parent: 'parentId', value: 'id', label: 'deptName', children: 'children' }},
          { type: 'Select', label: '岗位', prop: 'postId', rules: { required: true }, props: { label: 'postName', value: 'id' }},
          { type: 'Input', label: '用户邮箱', prop: 'userEmail', rules: { required: false, maxLength: 50, type: 'email' }},
          { type: 'Input', label: '座机', prop: 'userPhone', rules: { required: false, maxLength: 15, type: 'phone' }},
          { type: 'Input', label: '手机', prop: 'userMobile', rules: { required: true, maxLength: 20, type: 'mobile' }},
          // {type:'Select',label:'是否管理员',prop:'isAdmin',rules:{required:true},options:this.selectUtil.yesNo,change:this.changeIsAdmin},
          { type: 'Select', label: '角色', prop: 'roleId', rules: { required: true, maxLength: 20 }, props: { label: 'roleName', value: 'id' }}
          // {type:'MultiTreeSelect',label:'所属部门',prop:'deptIds',rules:{required:true},lazy:false,clearable:true,ref:"multiTreeSelect",data:[],valueProp:"deptId",props:{ parent: 'parentId', value: 'id',label: 'deptName',children: 'children',}},
        ],
        // modal表单 end
        // modal 数据 start
        modalData: { // modal页面数据
  ...
  methods: {
    ...
    toCode(str) { // 加密字符串
      ...
      var s = ''; var b; var b1; var b2; var b3 // 定义临时变量
    ...
    getDepts() {
      ...
      this.commonUtil.getTableList(obj).then(response => {
        ...
        // that.pageData.modalForm[9].data = response.responseData
      })
    },
    getPosts() {
      var obj = {
