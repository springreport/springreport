        ...
        tableCols:[
          ...
          {label:'数据源名称',prop:'dataSourceName',align:'center',overflow:true},
					{label:'操作',prop:'operation',align:'center',type:'button',btnList:[
						{label:'查看',type:'text',auth:'docTpl_getDetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'text',auth:'docTpl_edit',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
            {label:'设计',type:'text',auth:'docTpl_design',handle:(row)=>this.routerTo('docDesign',row)},
            {label:'报表查看',type:'text',auth:'docTpl_view',handle:(row)=>this.routerTo('docPreview',row)},
						{label:'删除',type:'text',auth:'docTpl_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
