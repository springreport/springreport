  ...
  data() {
    return{
      pageData:{
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',position: 'right', iconClass: 'action-icon-add',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'sysApi_insert'},
          {label:'批量删除',position: 'left', iconClass: 'action-icon-del',type:'danger',handle:()=>this.deleteBatch(),auth:'sysApi_batchDelete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
  ...
  methods:{
    ...
    searchtablelist(){
      var obj = {
        ...
        params:Object.assign({menuId:this.$store.getters.parameters['menuId']}, this.pageData.queryData, this.pageData.tablePage),
      }
    ...
    save(){
      this.$refs['modalRef'].$refs['modalFormRef'].validate((valid) => {
        if (valid) {
            ...
            obj.params.menuId = this.$store.getters.parameters['menuId'];
