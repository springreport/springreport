...
export default {
  data() {
    return {
      ...
      scaleList: [
        200, 150, 120, 100, 95, 90, 85, 80, 75, 70, 65, 60, 50, 40, 30, 20, 10
      ],
      scaleSelected: 75,
      screenProperties: {
        // 大屏属性
  ...
  computed: {
    ...
    componentsType() {
      ...
      return this.comKeyword
        ? comArr.filter((item) =>
          item.text.toLowerCase().includes(this.comKeyword.toLowerCase())
        )
        : comArr
    }
  },
  ...
  mounted() {
    ...
    VChart.ThemeManager.registerTheme(
      'veODesignDarkFinance',
      veODesignDarkFinance
    )
    VChart.ThemeManager.registerTheme(
      'veODesignDarkGovernment',
      veODesignDarkGovernment
    )
    VChart.ThemeManager.registerTheme(
      'veODesignDarkConsumer',
      veODesignDarkConsumer
    )
    VChart.ThemeManager.registerTheme(
      'veODesignDarkAutomobile',
      veODesignDarkAutomobile
    )
    VChart.ThemeManager.registerTheme(
      'veODesignDarkCulturalTourism',
      veODesignDarkCulturalTourism
    )
    VChart.ThemeManager.registerTheme(
      'veODesignDarkMedical',
      veODesignDarkMedical
    )
    VChart.ThemeManager.registerTheme(
      'veODesignDarkNewEnergy',
      veODesignDarkNewEnergy
    )
  ...
  methods: {
    ...
    openRenameVisible(item) {
      ...
      this.renameForm.comName =
        item.text || this.screenConstants.compType[item.type].text
    },
    // 确定更改
    sureRename() {
      this.renameVisible = false
    ...
    subComponentsSrc(subComponents) {
      return (
        subComponents[this.subComponentIndex]?.src +
          '?t=' +
          new Date().getTime() ?? ''
      )
    },
    ...
    useSubComponent(item, index) {
      ...
      // 关闭弹框
      this.$refs['subPopover' + index][0]?.doClose()
    },
    // 往画板中添加组件
        ...
        if (obj) {
          ...
          if (item.category == this.screenConstants.category.vchart) {
            if (obj.type.toLowerCase().indexOf('map') >= 0) {
              ...
              const categoryField = obj.spec.categoryField
              const valueField = obj.spec.valueField
      ...
      this.commonUtil.doPost(obj).then((response) => {
        if (response.code === '200') {
          ...
          if (
            ...
            for (
              let index = 0;
              index < response.responseData.components.length;
              index++
            ) {
    ...
    async preview() {
      ...
      const viewReport = this.$router.resolve({
        ...
        query: {
          ...
          sendRequest: 2,
          thirdPartyType: localStorage.getItem(
