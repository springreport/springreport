  ...
  data() {
    return {
      pageData: {
        ...
        // 表格工具栏按钮 start
        tableHandles: [
          { label: '新增', type: 'primary', position: 'right', iconClass: 'action-icon-add', handle: () => this.showModal(this.commonConstants.modalType.insert), auth: 'sysMerchantAuth_insert' },
          { label: '批量删除', type: 'danger', position: 'left', iconClass: 'action-icon-del', handle: () => this.deleteBatch(), auth: 'sysMerchantAuth_delete' }
        ],
        // 表格工具栏按钮 end
        selectList: [], // 表格选中的数据
        // 表格分页信息start
        ...
        tableCols: [
          { label: '操作', prop: 'operation', align: 'center', type: 'dropdown', width: 54, btnList: [
            { label: '查看', type: 'text', auth: 'sysMerchantAuth_getDetail', handle: (row) => this.showModal(this.commonConstants.modalType.detail, row.id) },
            { label: '编辑', type: 'text', auth: 'sysMerchantAuth_edit', handle: (row) => this.showModal(this.commonConstants.modalType.update, row.id) },
            { label: '删除', type: 'text', auth: 'sysMerchantAuth_delete', handle: (row) => this.deleteOne(row.id) }
          ] },
        ...
        // modal表单 start
        modalForm: [
          { type: 'Input', label: '模板名称', prop: 'templateName', rules: { required: true, maxLength: 50 }},
          { type: 'Tree', data: [], checked: [], props: { children: 'children', label: 'menuName' }, key: 'menuId', ref: 'authTree', label: '菜单权限', width: '200px' }
        ],
        // modal表单 end
        // modal 数据 start
        modalData: { // modal页面数据
