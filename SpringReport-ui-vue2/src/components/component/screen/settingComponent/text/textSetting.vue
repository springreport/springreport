    ...
    <!-- <el-collapse>
      <el-collapse-item :title="component.type != 'date'?'文本设置':'日期设置'"> -->
        <el-form ref="settingForm" class="demo-form-inline" :model="component" label-position="top" size="mini">
          ...
          <el-form-item v-if="component.type == 'date'" label="日期格式">
          ...
          <el-form-item label="字体起始颜色">
            <input-color-picker :input-width="152" :value="component.style.color" @change="(val)=>{component.style.color=val;changeFontColor(1)}" />
          </el-form-item>
          <el-form-item label="字体结束颜色">
            <input-color-picker :input-width="152" :value="component.style.colorEnd" @change="(val)=>{component.style.colorEnd=val;changeFontColor(2)}" />
          </el-form-item>
          <el-form-item label="渐变方向">
            <el-select v-model="component.style.direction" placeholder="请选择">
              <el-option
          ...
          <el-form-item label="起始背景色">
            <input-color-picker :input-width="152" :value="component.style.background" @change="(val)=>{component.style.background=val;changeBackgroundColor(1)}" />
          </el-form-item>
          <el-form-item label="结束背景色">
            <input-color-picker :input-width="152" :value="component.style.backgroundEnd" @change="(val)=>{component.style.backgroundEnd=val;changeBackgroundColor(2)}" />
          </el-form-item>
          <el-form-item label="渐变方向">
            <el-select v-model="component.style.backgroundDirection" placeholder="请选择">
              <el-option
                v-for="item in selectUtil.lineargardient"
                :key="item.value"
                :label="item.label"
          ...
          <el-form-item label="是否加粗" class="df-form-item">
            <el-switch v-model="component.style.fontWeight" active-text="是" inactive-text="否" />
          </el-form-item>
